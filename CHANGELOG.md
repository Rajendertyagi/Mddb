...
### Changed
- **GO-015 (stage 1) — extract the first `internal/` package from the flat `package main`** ([services/mddbd/internal/cache/](services/mddbd/internal/cache/)) — the `mddbd` daemon is one flat ~58k-LOC `package main` with no compilation boundaries. As the first step of the audit's prescribed gradual (no-big-bang) refactor, the read-through document cache (`cache.go` + tests) moved to an importable **`mddb/internal/cache`** package. It's a clean leaf — `DocumentCache` depends only on the standard library, not the `Server` god-object — so the extraction is safe: ~20 `package main` files were rewired to `cache.DocumentCache`/`NewDocumentCache`/`BuildCacheKey`, and the full test suite (incl. the new `mddb/internal/cache` package) stays green with coverage intact. Further leaves (indexqueue, fts, vector), transport adapters, the god-object split, and a shared client package will follow in subsequent stages.
